...
namespace CodeSubmissionSimple.Server.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
		...
		public async Task<LoginResult> Post([FromBody] RegisterModel reg)
		{
			...
			if (newuser != null)
				return new LoginResult { Message = "Registration successful.", JwtBearer = CreateJWT(newuser), 
					Email = reg.Email, Role = reg.Role, Success = true };
		...
		public async Task<LoginResult> Post([FromBody] LoginModel log)
		{
			...
			if (user != null)
				return new LoginResult { Message = "Login successful.", JwtBearer = CreateJWT(user), 
					Email = log.Email, Role = user.Role, Success = true };
